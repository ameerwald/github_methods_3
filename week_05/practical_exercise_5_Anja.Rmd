...
```{r}
#df$seed <- ifelse(df$seed == "o"| df$obj.resp == "e" & df$target.type == "even", 1, 0)
...
```{r}
df_no_practice <- df %>% 
  filter(trial.type == "staircase", "experiment")
```
...
model.5.1_partial <- glmer(correct ~ target.frames + (1 | subject), data = df, family = binomial(link = "logit")) #partial pooling?

model.5.1_pooling <- lm(corect ~ target.frames, data = df, family = binomial)
