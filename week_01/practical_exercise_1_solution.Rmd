...
1. plot the fitted values for __logistic.model__:  
    ...
    The  __linear.predictors__ a set of values or given data used to predict the dependent variable. In this case, our linear predictor is weight = -4.023 which is use to predict the outcome variable of transmission (am). The fitted values are the result of the linear predicts within the logistic model, that fall within the range 0-1.  
...
# we've identified the coeffiences but are unusre how to plot the function based on them 
