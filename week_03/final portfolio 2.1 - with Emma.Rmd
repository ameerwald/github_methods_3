...
ggplot(df.1.2.3, aes(x = target.contrast, y = fitted_correct, color = subject)) +
  ...
  facet_wrap(~ subject) +
  theme_bw()
## made a change here from the original to color by subject 

```
The fits are bad - not enough data to plot the functions. Don't follow a sigmoid function 
...
# Emma compared models...why didn't we? Had the lowest numbers for her, makes sense to adjust by task and trial  
