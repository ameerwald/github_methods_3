...
ggplot(df.1.2.3, aes(x = target.contrast, y = fitted_correct, color = subject)) +
  geom_point(aes(target.contrast, fitted_correct)) +
  facet_wrap(~ subject) +
  theme_bw()
 

```
The fits are bad - not enough data to plot the functions. Doesn't follow a sigmoid function 
...
- it illustrates individual variance more
 models both general tendency plus taking into account individual differences, accounts for the nested structure of our data
...
## Conceptually what's the difference between these two? 
model.2.1.1 <- lmer(rt.obj ~ 1 + (1 | subject) , data = df_4participants)
model.2.1.1

model.2.1.2 <- lm(rt.obj ~ 1, data = df_4participants)
...
# Had the lowest numbers for her, makes sense to adjust by task and trial  

#@ model.2.2.1 <- lmer(rt.obj ~ task + (1 + task| subject) + (1 | trial), df_experiment, REML=FALSE)
...
### ii. explain in your own words what your chosen models says about response times between the different tasks
 ...
 - singles has the fastest response time and quadruplet is close behind with pairs being the slowest 
...
#@ Emma useD her model from above  lmer(rt.obj ~ task*pas + (1 + task| subject) + (1 | trial), df_experiment, REML=FALSE)
```
...
model.2.3.1
#@ we have singular fit already from 2 intercepts <- why adding an intercept for correct??
```
...
### iii. in your own words - how could you explain why your model would result in a singular fit?
 ...
 - because our second intercept is zero, indicates model is overfitting and structure is too complex
...
data.count$count <-  as.numeric(data.count$count) # < makes more sense at integer b/c can't have a decimal count 
...
### ii. why is a slope for _pas_ not really being modelled?  
  #@ NEVER ANSWERED THIS   
Because encoded as a factor, it calculates for each level of pas and not as a whole. 
...
# data.count$task <- relevel(data.count$task, ref = "singles") - should change the order of the scatterplots 
levels(data.count$task)

ggplot(data.count, aes(col=pas, x = count)) + geom_density() + facet_wrap(~task)
# from sigurd - another nice way to visualize 
...
#@ Emma's probably not necessary but nice to see the contract between estimated and observed 
data.count_2 %>% 
  ggplot() +
   geom_point(aes(x = pas, y = fitted_correct_2, color = "Estimated")) +
...
model.3.3.0 <- glmer(correct ~ task + (1|subject), data = df, family = binomial)

```

### i. does _task_ explain performance?  
```{r}
#@ come back here to fix the explanation based on fixed model 
...
```{r}
model.3.3.2 <- glmer(correct ~ task + pas + (1|subject), data = df, family = binomial)
...
```{r}
model.3.3.3 <- glmer(correct ~ pas + (1|subject), data = df, family = binomial)
...
```{r}
model.3.3.4 <- glmer(correct ~ pas*task + (1|subject), data = df, family = binomial)
